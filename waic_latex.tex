...
% -- Add the following lines for a draft watermark on every page --
\usepackage{draftwatermark}        % Loads the watermark package
\SetWatermarkText{DRAFT v0.11}           % Sets the watermark text
\SetWatermarkScale{0.9}              % Adjusts the size of the watermark; tweak as needed
\SetWatermarkLightness{0.93}       % Sets the lightness so it is subtle; adjust between 0 (dark) and 1 (light)
% -------------------------------------------------------------
...
\begin{definition}[Declarative Program]\label{def:declarative-program}
A \textit{declarative program} $f$ is any subset of $\Phi$, i.e. $f \subseteq \Phi$.
\end{definition}
...
\begin{definition}[Abstraction Layer]\label{def:abstraction-layer}
...
\begin{definition}[Intervention and Causal Identity]\label{def:causal-identity}
...
\begin{theorem}[Substrate Independence of Consciousness]
